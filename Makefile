...
PROJECT_NAME=design-plone-opendata
STACK_NAME=www-iocomune-it
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://design-plone-opendata.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t RedTurtle/design-plone-opendata-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t RedTurtle/design-plone-opendata-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name design-plone-opendata-frontend-acceptance --link design-plone-opendata-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d RedTurtle/design-plone-opendata-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name design-plone-opendata-backend-acceptance -d RedTurtle/design-plone-opendata-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop design-plone-opendata-frontend-acceptance
	@docker stop design-plone-opendata-backend-acceptance
